...
bool Programmer::PicBase::readRange(Pic::MemoryRangeType type, Pic::Memory *memory, const VerifyData *vd)
{
  ...
  if ( !specific()->canReadRange(type) ) {
    log(Log::LineType::Information, i18n("The selected programmer cannot read %1: operation skipped.").arg(i18n(type.label())));
    return true;
  }
  VerifyData *vdata = (vd ? new VerifyData(vd->actions, vd->memory) : 0);
  if (vdata) {
    log(Log::LineType::Information, i18n("  Verify memory: %1").arg(i18n(type.label())));
    ...
    log(Log::LineType::Information, i18n("  Read memory: %1").arg(i18n(type.label())));
    CRASH_ASSERT(memory);
  }
  Device::Array data;
...
{
  log(Log::LineType::Information, i18n("  Write memory: %1").arg(i18n(mtype.label())));
...
{
  log(Log::LineType::Information, i18n("  Write memory: %1").arg(i18n(Pic::MemoryRangeType(Pic::MemoryRangeType::Cal).label())));
  success = true;
  ...
  if ( device()->isWritable(Pic::MemoryRangeType::CalBackup) ) {
    ...
    log(Log::LineType::Information, i18n("  Write memory: %1").arg(i18n(Pic::MemoryRangeType(Pic::MemoryRangeType::CalBackup).label())));
