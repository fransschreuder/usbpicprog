...
case $AUTOCONF_VERSION in
  Autoconf*2.5* | autoconf*2.5* | Autoconf*2.6* | autoconf*2.6* ) echo "*** $AUTOCONF_VERSION found." ;;
  "" )
...
case $AUTOHEADER_VERSION in
  Autoconf*2.5* | autoheader*2.5* | Autoconf*2.6* | autoheader*2.6* ) echo "*** $AUTOHEADER_VERSION found." ;;
  "" )
...
for subdir in $dirs; do
  ...
  (cd $subdir
   ...
   $MAKE -s -f _transMakefile podir=$podir EXTRACTRC="$EXTRACTRC" PREPARETIPS="$PREPARETIPS" srcdir=. \
        XGETTEXT="${XGETTEXT:-xgettext} --foreign-user -C -ci18n -ki18n -ktr2i18n -kI18N_NOOP -kI18N_NOOP2 -kaliasLocale -x $kdepotpath -s" messages
   exit_code=$?
