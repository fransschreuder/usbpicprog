...
{
  switch (status) {
    case Device::Status::Future:         return Qt::blue;
    case Device::Status::InProduction:   return Qt::green;
    case Device::Status::Mature:
    case Device::Status::NotRecommended: return QColor("orange");
    case Device::Status::EOL:            return Qt::red;
    case Device::Status::Unknown:
    case Device::Status::Nb_Types: break;
  }
  return Qt::black;
}
...
QString Device::htmlInfo(const Device::Data &data, const QString &deviceHref, const QString &documentHtml)
{
  ...
  doc += "<table>";
  QString status = coloredString(i18n(data.status().label()), statusColor(data.status()));
...
QString Device::htmlVoltageFrequencyGraphs(const Device::Data &data, const QString &imagePrefix, QMimeSourceFactory *msf)
{
  ...
  FOR_EACH(Special, special) {
    for (uint k=0; k<data.frequencyRanges().count(); k++) {
      ...
      doc += "<h3>" + data.fname(special) + " - " + i18n("Temperature range: ")
    + i18n(fr.operatingCondition.label()) + "</h3>";
