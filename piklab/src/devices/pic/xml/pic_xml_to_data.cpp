...
void processName(const Pic::Config::Mask &cmask, BitValue pmask, Pic::Config::Value &cvalue)
{
  if ( cvalue.name=="invalid" ) {
    ...
    if ( !cvalue.cnames.isEmpty() ) qFatal(QString("No cname should be defined for invalid value in mask %1").arg(cmask.name));
    return;
  }
  ...
  if ( cmask.value.isInside(pmask) ) { // protected bits
    if ( !cvalue.cnames.isEmpty() ) qFatal(QString("Config name should be null for protected config mask \"%1\"").arg(cmask.name));
  } else {
    if ( cvalue.cnames.isEmpty() && cmask.name!="BSSEC" && cmask.name!="BSSIZ" && cmask.name!="SSSEC" && cmask.name!="SSSIZ" ) {
    ...
    if ( cvalue.cnames.count()==1 && cvalue.cnames[0]=="_" ) cvalue.cnames.clear();
    for (uint i=0; i<uint(cvalue.cnames.count()); i++) {
      if ( cvalue.cnames[i].startsWith("0x") ) {
        if ( cvalue.cnames.count()!=1 ) qFatal("Hex cname cannot be combined");
        bool ok;
        BitValue v = fromHexLabel(cvalue.cnames[i], &ok);
        uint nbChars = data()->nbCharsWord(MemoryRangeType::Config);
        BitValue mask = cmask.value.complementInMask(maxValue(Number::Hex, nbChars));
        if ( ok && v==(mask | cvalue.value) ) continue;
      } else if ( XOR(cvalue.cnames[i].startsWith("_"), data()->architecture()==Pic::Architecture::P30X) ) continue;
...
Pic::Config::Mask toConfigMask(QDomElement mask, BitValue pmask)
{
  ...
  bool ok;
  QString defName;
  QStringList defCnames, defECnames;
  Config::Mask cmask;
  ...
  while ( !child.isNull() ) {
    ...
    if ( value.attribute("value")=="default" ) {
      ...
      defCnames = QStringList::split(' ', value.attribute("cname"));
      defECnames = QStringList::split(' ', value.attribute("ecname"));
      continue;
    }
    Config::Value cvalue;
    ...
    cvalue.cnames = QStringList::split(' ', value.attribute("cname"));
    cvalue.extraCnames = QStringList::split(' ', value.attribute("ecname"));
  ...
  if ( !defName.isEmpty() ) {
    ...
    for (it=cmask.value.begin(); it!=cmask.value.end(); ++it) {
      ...
      cvalue.value = *it;
      cvalue.name = defName;
      cvalue.cnames = defCnames;
      cvalue.extraCnames = defECnames;
      processName(cmask, pmask, cvalue);
      cmask.values.append(cvalue);
    }
...
Pic::Config::Word toConfigWord(QDomElement config)
{
  ...
  if ( config.attribute("cmask").isEmpty() ) {
    if ( data()->_architecture==Pic::Architecture::P30X ) cword.cmask = cword.wmask;
...
virtual void processDevice(QDomElement device)
{
  ...
  if ( data()->_architecture==Architecture::P10X ) {
  ...
  for (uint i=0; i<nbWords; i++) {
    ...
    for (uint j=0; j<uint(word.masks.count()); j++) {
      ...
      for (uint k=0; k<uint(mask.values.count()); k++) {
        for (uint l=0; l<uint(mask.values[k].cnames.count()); l++) {
          QString cname = mask.values[k].cnames[l];
          if ( cname.startsWith("0x") ) continue;
