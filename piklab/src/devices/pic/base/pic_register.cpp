...
bool Pic::RegistersData::hasPort(uint index) const
{
  ...
  if ( _data.architecture()==Architecture::P10X ) return ( index==0 );
...
{
  if ( _data.architecture()==Architecture::P10X || sfrs.contains("GPIO") ) {
...
{
  if ( _data.architecture()==Architecture::P10X ) {
    if ( index!=0 ) return QString::null;
    return "TRISIO";
  }
