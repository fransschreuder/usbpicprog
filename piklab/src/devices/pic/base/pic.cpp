...
//  {Code, Cal, UserID, DevId, Conf, EEPROM, DebugVec, HardStack, CalBackup, Program Executive} randomAccess
  ...
  { "17C",  I18N_NOOP("17C Family"),      16, 2, false,  8, 0x100, { 16,  0,  0,  0, 16,  8,  0,  0,  0,  0 }, true  },
  ...
  { "24J",  I18N_NOOP("24J Family"),      23, 4, true,  16, 0x800, { 24,  0, 16, 16, 16,  0, 24,  0,  0, 24 }, true  },
...
bool Pic::Data::isGotoInstruction(BitValue instruction) const
{
  ...
  case Architecture::P24J:
  case Architecture::P24H:
  case Architecture::P30X: return ( instruction.maskWith(0xFF0000)==0x040000 );
  case Architecture::Nb_Types: Q_ASSERT(false); break;
  }
  return false;
}
...
QDataStream &Pic::operator <<(QDataStream &s, const Pic::Data &data)
{
  ...
  FOR_EACH(ProgVoltageType, i) s << data._voltages[i];
  FOR_EACH(MemoryRangeType, i) s << data._ranges[i];
  s << *data._config;
  s << data._checksums;
  s << data._calibration;
