...
Register::ListViewItem::ListViewItem(const TypeData &data, KListViewItem *parent)
  : EditListViewItem(parent), _data(data), _base(Number::Hex)
{
  setSelectable(false);
  const Device::RegistersData *rdata = Main::deviceData()->registersData();
...
QString Register::ListViewItem::text() const
{
  ...
  uint nbChars = convertNbChars(_data.nbChars(), Number::Hex, _base);
...
void Register::ListViewItem::setBase(Number::Base base)
{
  _base = base;
  updateView();
}
...
Register::LineEdit::LineEdit(QWidget *parent, const char *name)
  : Number::LineEdit(parent, name), _base(Number::Nb_Bases)
{
...
void Register::LineEdit::setValue(Number::Base base, BitValue value, uint nbChars)
{
  _base = base;
  _value = value;
  _nbChars = nbChars;
