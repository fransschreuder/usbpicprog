...
{
  log(Log::NormalDebug, QString("  Calibration firmware file: %1").arg(url.pretty()));
  ...
  if ( !memory.load(file.stream(), errors, warningTypes, warnings) ) {
    log(Log::Error, i18n("Could not read calibration firmware file \"%1\" (%2).").arg(url.pretty()).arg(errors[0]));
    return false;
  }
