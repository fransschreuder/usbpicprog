...
void Compile::LogWidget::appendLine(Log::Type type, const QString &message, const QString &filepath, uint line)
{
...
{
  return Main::toolGroup().check(_data.device, _manager);
}
...
{
  return url(type, i).relativeTo(directory(), Main::toolGroup().withWine() ? PURL::WindowsSeparator : PURL::UnixSeparator);
}
  ...
  for (uint i=0; i<args.count(); i++) {
    ...
    if ( checkIs(args[i], "%OBJS") ) { // before %O
      if (_data.project) nargs += _data.project->objectsForLinker(objectExtension(), Main::toolGroup().withWine());
        ...
        nargs += tmp.relativeTo(directory(), Main::toolGroup().withWine() ? PURL::WindowsSeparator : PURL::UnixSeparator);
      }
      continue;
    }
    if ( checkIs(args[i], "%LIBS") ) {
      if (_data.project) nargs += _data.project->librariesForLinker(QString::null, Main::toolGroup().withWine());
      continue;
    }
    ...
    args[i].replace("%SYM", url().toExtension("sym").relativeTo(directory(), Main::toolGroup().withWine() ? PURL::WindowsSeparator : PURL::UnixSeparator));
...
bool Compile::Process::start()
{
  ...
  QString exec = tool()->baseExecutable(Main::toolGroup().withWine(), Main::toolGroup().outputType());
  QString path = tool()->executableDirectory().path();
  _process->setup(path + exec, arguments(), Main::toolGroup().withWine());
...
void Compile::Manager::log(Log::Type type, const QString &message, const QString &filepath, uint line)
{
