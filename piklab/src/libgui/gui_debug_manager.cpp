...
void Debugger::GuiManager::updateEditorMarks(TextEditor &editor) const
{
  ...
  int pcline = (isDebugging && pc().isInitialized() ? _coff->lineForAddress(editor.url(), pc().toUInt()) : -1);
  if ( pcline!=-1 && !reached ) {
...
{
  if ( _coff==0 ) return;
  int pcline = _coff->lineForAddress(editor.url(), pc().toUInt());
  if ( pcline==-1 ) return;
  shown = &editor;
...
void Debugger::GuiManager::showPC()
{
  ...
  if ( shown==0 ) {
    PURL::Url url = _coff->urlForAddress(pc().toUInt());
...
void Debugger::GuiManager::updateView(bool gotoPC)
{
  ...
  if ( programmer() && programmer()->state()==Programmer::Halted ) {
    if ( shown==0 && _coff && pc().isInitialized() ) {
      PURL::Url url = _coff->urlForAddress(pc().toUInt());
      if ( !url.exists() ) return;
      TextEditor *e = ::qt_cast<TextEditor *>(Main::editorManager().openEditor(url));
      if (e) {
