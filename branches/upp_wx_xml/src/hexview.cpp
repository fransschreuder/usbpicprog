...
void UppHexViewGrid::OnCopy (wxCommandEvent& WXUNUSED(event))
{
    Copy();
}

void UppHexViewGrid::OnSelectAll (wxCommandEvent& WXUNUSED(event))
{
    SelectAll();
}

void UppHexViewGrid::OnCellRightClicked (wxGridEvent& WXUNUSED(event))
{
    wxMenu oMenu(_("Edit"));
    wxPoint oPos;
