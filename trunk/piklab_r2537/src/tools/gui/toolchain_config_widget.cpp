...
void ToolchainConfigWidget::init()
{
  ...
  if ( !_group.comment().isEmpty() ) {
    ...
    _topLayout->addMultiCellWidget(w, row,row, 0, 3);
    row++;
  }
...
void ToolchainConfigWidget::loadConfig()
{
  ...
  if (_outputType) {
    ...
    _outputType->setCurrentItem(Compile::Config::outputType(_group).key());
...
void ToolchainConfigWidget::saveConfig()
{
  ...
  if (_outputType) Compile::Config::setOutputType(_group, outputType());
  FOR_EACH(Compile::DirectoryType, type)
...
{
  if ( _outputType==0 ) return Compile::Config::outputType(_group);
