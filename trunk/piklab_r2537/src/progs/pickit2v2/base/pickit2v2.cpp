...
bool Pickit2V2::Hardware::readMemory(Pic::MemoryRangeType otype, Device::Array &data, const ::Programmer::VerifyData *vdata)
{
  ...
  if ( otype==Pic::MemoryRangeType::Eeprom ) nbBytesWord = 2; // EEPROM is read like regular program memory in midrange
  // ### TODO: totally untested with PIC18s and PIC24/dsPIC
  uint wordOffset = 0;
  ...
  for (uint i=0; i<nbWords; ) {
    ...
    if ( type==Pic::MemoryRangeType::Config || type==Pic::MemoryRangeType::Cal ) {
      ...
      if ( !port().receiveWords(nbBytesWord, nbReceive, words, 3) ) return false;
    } else {
    ...
    for (uint k=0; k<words.count(); k++) {
      ...
      if ( type==Pic::MemoryRangeType::Code && i%0x8000==0 ) setAddress = true;
      i++;
    }
  }
