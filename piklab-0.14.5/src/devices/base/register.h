...
public:
  ...
  virtual uint indexFromAddress(uint address) const = 0;
  virtual RegisterProperties properties(uint address) const = 0;
  ...
  virtual int portIndex(uint address) const = 0;
...
public:
  ...
  void setValue(const TypeData &data, uint value);
  uint value(const TypeData &data) const;
  uint oldValue(const TypeData &data) const;
...
private:
  class StateData {
  public:
    StateData() : current(Device::INVALID), old(Device::INVALID) {}
    uint current, old;
  };
  QMap<uint, StateData>    _regulars; // registers with address
