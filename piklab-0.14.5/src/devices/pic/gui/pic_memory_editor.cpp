  ...
  : MemoryEditor(type, memory, parent, "memory_displayer_legend")
{
  ...
  QString s = (protection.hasCodeGuard() ? i18n("High Security") : i18n("Code protection"));
  QLabel *label = new QLabel(s, this);
  grid->addMultiCellWidget(label, 0,0, 1,2);
  grid->addRowSpacing(1, 10);
  uint row = 2;

  if ( type==MemoryCode && protection.hasBootBlock() ) {
    w = new QWidget(this);
    w->setFixedWidth(20);
  ...
  uint nb = (type==MemoryCode ? protection.nbBlocks() : 0);
  for (uint i=0; i<nb; i++) {
    w = new QWidget(this);
    w->setFixedWidth(20);
...
{
  const Protection &protection = device().config().protection();
  Protection::Type ptype = (protection.hasCodeGuard() ? Protection::HighSecurity : Protection::ProgramProtected);
  uint nbChars = 2 * device().nbBytesAddress();
  if (_boot.label) {
    Range r = memory().bootRange();
...
void Pic::HexWordEditor::setWord(uint value)
{
  if ( type()==MemoryConfig ) {
    const Config::Word &cword = device().config()._words[_offset];
    value |= ~cword.usedMask();
    value &= device().mask(MemoryConfig);
  }
...
{
  if ( type==MemoryCode ) _blockRanges.resize(memory.device().config().protection().nbBlocks());
}

void Pic::MemoryRangeEditor::addLegend(QVBoxLayout *vbox)
{
  if ( type()==MemoryCode || type()==MemoryEeprom ) {
...
{
  return ( (MEMORY_RANGE_TYPE_DATA[type()].properties & ReadOnly) || type()==MemoryCalBackup );
}
...
void Pic::MemoryRangeEditor::updateAddressColor(uint i, uint address)
{
...
void Pic::MemoryTypeEditor::init(bool first)
{
  ...
  _title->setText(i18n(MEMORY_RANGE_TYPE_DATA[type()].label));

  uint nbChars = device().nbCharsWord(type());
  const MemoryRangeData &mr = device().range(type());
  QString add;
  if ( type()==MemoryUserId ) add = i18n(" - read mask: %1").arg(toHexLabel(mr.rmask, nbChars));
  if ( type()==MemoryCal && _hexview ) add = i18n(" - not programmed by default");
...
void Pic::MemoryTypeRangeEditor::init(bool first)
{
  ...
  uint nbLines = 0;
  if ( type()==MemoryCode ) nbLines = 16;
  else if ( type()==MemoryEeprom ) nbLines = 8;
...
Pic::MemoryUserIdEditor::MemoryUserIdEditor(const HexView *hexview, Memory &memory, QWidget *parent)
  : MemoryTypeRangeEditor(hexview, MemoryUserId, memory, parent), _saveReadOnly(false)
{}
...
void Pic::MemoryUserIdEditor::init(bool first)
{
  ...
  GlobalConfig gc;
  if ( gc.isUserIdSetToChecksum() && memory().isClear(MemoryUserId) ) {
...
Pic::MemoryCalibrationEditor::MemoryCalibrationEditor(const HexView *hexview, Memory &memory, QWidget *parent)
  : MemoryTypeEditor(hexview, MemoryCal, memory, parent, "pic_memory_calibration_editor")
{}

void Pic::MemoryCalibrationEditor::init(bool first)
{
  MemoryTypeEditor::init(first);
  MemoryRangeEditor *mre = new MemoryRangeEditor(MemoryCal, memory(), 1, 8, 0, -1, this);
  addEditor(mre);
  _top->addWidget(mre);
  mre->init();
  if ( device().isReadable(MemoryCalBackup) ) {
    mre = new MemoryRangeEditor(MemoryCalBackup, memory(), 1, 8, 0, -1, this);
    addEditor(mre);
    _top->addWidget(mre);
    mre->init();
...
bool Pic::MemoryCalibrationEditor::internalDoAction(Device::Action action)
{
  ...
  switch (action) {
    case Device::Reload: {
      const Memory *omemory = static_cast<const Memory *>(originalMemory());
      Q_ASSERT(omemory);
      memory().copyFrom(MemoryCal, *omemory);
      memory().copyFrom(MemoryCalBackup, *omemory);
      return true;
    }
    case Device::Program:
      if ( prog->programCalibration(memory().arrayForWriting(Pic::MemoryCal)) )
        return prog->readSingle(MemoryCal, memory());
      return false;
    case Device::Verify:
      prog->verifySingle(MemoryCal, memory());
      return false;
    case Device::Read:
      return prog->readSingle(MemoryCal, memory());
    case Device::Clear:
