...
private:
  virtual uint mask() const { return memory().device().mask(type()); }
  virtual uint normalizeWord(uint value) const { return memory().normalizeWord(type(), _offset, value); }
  virtual uint word() const { return memory().word(type(), _offset); }
  virtual void setWord(uint value);
};
...
private:
  ...
  Range       _bootRange;
  RangeVector _blockRanges;
  RangeVector _codeProtected;
  ...
  virtual uint startAddress() const { return device().range(type()).start; }
  virtual Device::HexWordEditor *createHexWordEditor(QWidget *parent);
  virtual void updateAddressColor(uint i, uint address);
