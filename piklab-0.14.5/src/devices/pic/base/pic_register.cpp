...
uint Pic::RegistersData::mirroredAddress(uint address) const
{
  uint mirror = address;
...
Device::RegisterProperties Pic::RegistersData::properties(uint address) const
{
  switch ( type(address) ) {
    case Mirrored:
...
Pic::RegisterType Pic::RegistersData::type(uint address) const
{
  ...
  uint mirror = mirroredAddress(address);
  if ( !sfrNames[mirror].isEmpty() ) return Mirrored;
  if ( address==mirror ) return Gpr;
  return Mirrored;
}

QString Pic::RegistersData::label(uint address) const
{
  switch ( type(address) ) {
...
bool Pic::RegistersData::hasPort(uint index) const
{
  ...
  if ( _data.architecture()==Arch_10X ) return ( index==0 );
  if ( !sfrs.contains(portName(index)) ) return false;
  return true;
}

int Pic::RegistersData::portIndex(uint address) const
{
...
{
  if ( _data.architecture()==Arch_10X ) {
...
QString Pic::RegistersData::trisName(uint index) const
{
  ...
  if ( _data.architecture()==Arch_17X ) {
    if ( index==0 ) return QString::null;
...
{
  if ( _data.architecture()==Arch_10X || _data.architecture()==Arch_16X || _data.architecture()==Arch_17X )
    return QString::null;
...
QString Pic::RegistersData::portBitName(uint index, uint bit) const
{
  QString name;
  uint nb = 0;
  for (uint i=0; i<Device::MAX_NB_PORTS; i++) if ( hasPort(index) ) nb++;
  if ( nb==1 ) return QString("GP") + QString::number(bit);
...
{
  if ( i==0 ) return true;
  if ( (_data.architecture()==Pic::Arch_18F || _data.architecture()==Pic::Arch_18J) && i==15 ) return true;
...
bool Pic::RegistersData::hasSharedGprs(uint &firstIndex, bool &all) const
{
  ...
  for (uint i=0; i<nbRegistersPerBank(); i++) {
    uint address = addressFromIndex(i);
