...
bool Pic::Config::checkValueName(const QString &mask, const QString &name) const
{
  ...
  switch (data.type) {
    ...
    case Toggle:
      ...
      break;
    case Pin: {
      QRegExp regexp("[A-Z]+\\d*(?:/[A-Z]+\\d*|)*(?: \\(.*\\)|)");
      if ( regexp.exactMatch(name) ) return true;
      break;
    }
    case Pins: {
      QRegExp regexp("[A-Z]+\\d*(?:/[A-Z]+\\d*|)*(?: \\(.*\\)|)(, [A-Z]+\\d*(?:/[A-Z]+\\d*|)*(?: \\(.*\\)|))+");
      if ( regexp.exactMatch(name) ) return true;
      break;
    }
  }
...
uint Pic::Config::Word::usedMask() const
{
  uint mask = 0;
  for (uint i=0; i<uint(masks.count()); i++)
    mask |= masks[i].value;
  return mask;
}
...
{
  s << value.name << value.cname << value.extraCNames << value.value;
  return s;
}
