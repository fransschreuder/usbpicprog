...
public:
  PicHardware(::Programmer::Base &base, Port::Base *port) : Hardware(base, port) {}
  const Pic::Data &device() const { return static_cast<const Pic::Data &>(*_base.device()); }
  ...
  bool compareWords(Pic::MemoryRangeType type, uint index, uint v, uint d, VerifyActions actions);
  bool verifyWord(uint index, uint word, Pic::MemoryRangeType type, const VerifyData &vdata);
};
