...
private:
  virtual uint mask() const { return 0xFF; }
  virtual uint normalizeWord(uint value) const { return value; }
  virtual uint word() const { return memory().byte(_offset); }
  virtual void setWord(uint value) { memory().setByte(_offset, value); }
};
...
private:
  ...
  virtual uint startAddress() const { return 0; }
