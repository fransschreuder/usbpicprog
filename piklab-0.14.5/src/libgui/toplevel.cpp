...
ProgrammerStatusWidget::ProgrammerStatusWidget(QWidget *parent)
  : PopupButton(parent, "programmer_status_widget")
{
  connect(popup(), SIGNAL(activated(int)), SLOT(activatedSlot(int)));
  ...
  for (uint i=0; i<Programmer::lister().nbGroups(); i++) {
    ...
    appendItem(group.label(), i);
  }
}

void ProgrammerStatusWidget::activatedSlot(int i)
{
  if ( i<0 ) return;
  emit selected(Programmer::lister().group(i));
}

//----------------------------------------------------------------------------
ToolStatusWidget::ToolStatusWidget(QWidget *parent)
  : PopupButton(parent, "tool_status_widget")
{
  connect(popup(), SIGNAL(activated(int)), SLOT(activatedSlot(int)));
  ...
  for (uint i=0; i<Tool::lister().nbGroups(); i++) {
    const Tool::Group &group = Tool::lister().group(i);
    appendItem(group.label(), i);
  }
}

void ToolStatusWidget::activatedSlot(int i)
{
  if ( i<0 ) return;
  emit selected(Tool::lister().group(i));
}
...
{
  if (_pikloop) return;
  _pikloop = new ::Process::StringOutput;
  _pikloop->setup("pikloops", QStringList(), false);
  connect(_pikloop, SIGNAL(done(int)), SLOT(pikloopDone()));
  if ( !_pikloop->start(0) )
    MessageBox::detailedSorry(i18n("Could not run \"pikloops\""),
                              i18n("The Pikloops utility (http://pikloops.tripod.com) is not installed in your system:\n"
                                   " You can download pikloops from http://piklab.sourceforge.net"), Log::Show);
}

void MainWindow::pikloopDone()
{
  delete _pikloop;
  _pikloop = 0;
}
...
void MainWindow::configGenerator()
{
  ...
  PURL::SourceFamily family = (ftype!=PURL::Nb_FileTypes ? PURL::DATA[ftype].sourceFamily : PURL::Nb_SourceFamilies);
  PURL::ToolType type = (family!=PURL::Nb_SourceFamilies ? PURL::SOURCE_FAMILY_DATA[family].toolType : PURL::Assembler);
...
void MainWindow::newSourceFile()
{
  ...
  if ( !dialog.url().probablyExists() && !dialog.url().create(Main::_compileLog) ) return;
...
void MainWindow::updateGUI()
{
  ...
  Main::action("debug_show_pc")->setEnabled(idle && isDebugger && Debugger::manager->coff()!=0 && Debugger::manager->pc()!=Device::INVALID );
  ...
  if ( Main::currentEditor() ) {
    ...
    caption += Main::currentEditor()->url().filepath(false);
  }
  ...
  if ( !Main::_compileManager->compileOnly() ) {
    ...
    if ( Debugger::manager->init() ) {
      ...
      for (it=included.begin(); it!=included.end(); ++it) {
        ...
        PURL::Url url = PURL::Url(*it).toAbsolute(dir);
        if ( !url.probablyExists() ) continue;
        Main::_projectManager->addExternalFile(url, ProjectManager::Included);
      }
    }
    GlobalConfig gc;
    if ( gc.programAfterBuild() ) program();
  }
  Main::setState(Main::Idle);
}
...
HexEditor *MainWindow::getHexEditor()
{
  ...
  if ( Main::_projectManager->isModified() ) {
    if ( !MessageBox::askContinue(i18n("<qt>The project hex file may not be up-to-date since some project files have been modified. Continue anyway ?</qt>")) )
    return 0;
  }
