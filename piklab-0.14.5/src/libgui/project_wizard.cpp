...
PURL::FileGroup FileListItem::fileGroup() const
{
  ...
  case PURL::Source: {
    for (uint i=0; i<PURL::Nb_ToolTypes; i++)
      if ( _group->implementationType(PURL::ToolType(i))==url.fileType() ) return PURL::Source;
    break;
  }
...
{
  PURL::UrlList urls = PURL::getOpenUrls(_directory.path(false), QString::null, this, i18n("Select Files"));
  ...
  : KWizard(parent, "project_wizard"), _project(0)
{
  ...
  _toolchain = new KComboBox(_first);
  uint current = 0;
  for (uint i=0; i<Tool::lister().nbGroups(); i++) {
    const Tool::Group &group = Tool::lister().group(i);
    _toolchain->insertItem(group.label());
    if ( group.name()==Main::toolGroup().name() ) current = i;
  }
  _toolchain->setCurrentItem(current);
...
void ProjectWizard::done(int r)
{
  if ( r==Accepted ) {
    ...
    if ( _bgroup->selectedId()==0 ) {
      PURL::ToolType ttype = PURL::Compiler;
      PURL::FileType ftype = PURL::Nb_FileTypes;
      for (uint i=0; i<PURL::Nb_ToolTypes; i++) {
        PURL::FileType type = toolchain().implementationType(PURL::ToolType(i));
        if ( type==PURL::Nb_FileTypes ) continue;
        ftype = type;
        ttype = PURL::ToolType(i);
      }
...
{
  return static_cast<const Tool::Group &>(Tool::lister().group(_toolchain->currentItem()));
}
