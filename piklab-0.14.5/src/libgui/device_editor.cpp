...
PURL::Url DeviceEditor::findAsmFile(const PURL::Url &url)
{
  ...
  if ( PURL::SOURCE_FAMILY_DATA[family].toolType==PURL::Assembler ) return url;
  for (uint i=0; i<PURL::Nb_FileTypes; i++) {
    const PURL::Data &data = PURL::DATA[PURL::FileType(i)];
    if ( PURL::SOURCE_FAMILY_DATA[data.sourceFamily].toolType!=PURL::Assembler ) continue;
