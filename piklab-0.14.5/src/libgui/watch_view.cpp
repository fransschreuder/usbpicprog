...
Register::WatchView::WatchView(QWidget *parent)
  : ListView(parent, "watch_view"), GenericView(Register::list()),
    _popup(0), _formatPopup(0), _data(0), _format(Number::Hex)
{
...
void Register::WatchView::contextMenu(QListViewItem *item, const QPoint &p, int)
{
  ...
  if ( item==_root ) {
    if ( _popup==0 ) return;
    for (uint i=0; i<Number::Nb_Bases; i++)
      _formatPopup->setItemChecked(_ids.count()+i, _format==Number::Base(i));
    int res = _popup->exec(p);
    if ( res<0 ) return;
    if ( res<int(_ids.count()) ) Debugger::manager->setRegisterWatched(_ids[res], true);
    else {
      _format = Number::Base(res-_ids.count());
      for (QListViewItem *item=_root->firstChild(); item; item=item->nextSibling())
        static_cast<ListViewItem *>(item)->setFormat(_format);
    }
  } else {
    ...
    for (uint i=0; i<Number::Nb_Bases; i++)
      fpop->setItemChecked(i, ritem->format()==Number::Base(i));
    pop->insertSeparator();
    KIconLoader loader;
    ...
    else if ( res>=0 ) ritem->setFormat(Number::Base(res));
    delete pop;
  }
}
