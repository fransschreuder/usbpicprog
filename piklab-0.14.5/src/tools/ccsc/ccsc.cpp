...
QValueList<const Device::Data *> CCSC::Group::getSupportedDevices(const QString &) const
{
  ...
  for (uint i=0; i<devices.count(); i++) {
    ...
    if ( static_cast<const Pic::Data *>(data)->architecture()==Pic::Arch_30X ) continue;
    list.append(data);
  }
  return list;
}
...
Compile::Process *CCSC::Group::processFactory(const Compile::Data &data) const
{
  switch (data.category) {
    case Tool::Compiler: return new CCSC::CompileFile;
    default: break;
  }
  Q_ASSERT(false);
  return 0;
...
ToolConfigWidget *CCSC::Group::configWidgetFactory(Tool::Category category, Project *project) const
{
  if ( category==Tool::Compiler ) return new CCSC::ConfigWidget(project);
  return 0;
}
...
QString CCSC::Group::downloadText() const
{
  return i18n("<a href=\"http://www.ccsinfo.com/content.php?page=compilers\">CCS Compiler</a> is a C compiler distributed by CCS.");
}

Tool::Base *CCSC::Group::baseFactory(Tool::Category category) const
{
  if ( category==Tool::Compiler ) return new CCSC::Base;
  return 0;
}
...
VersionData CCSC::Group::getToolchainVersion()
{
  if ( !withWine() ) {
    QStringList lines;
    if ( checkExecutable(Tool::Compiler, lines) && lines.count()>=1 && lines[0].contains("3.") ) return VersionData(3, 0, 0);
  }
