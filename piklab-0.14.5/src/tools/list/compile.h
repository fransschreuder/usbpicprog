...
public:
  ...
  void appendLine(Log::Type type, const QString &message, const PURL::Url &url, uint line);
  void appendLine(Log::DebugLevel debug, const QString &message, const PURL::Url &url, uint line);
  virtual void clear();
...
private:
  ...
  public:
    Data() {}
    Data(const PURL::Url &u, uint l) : url(u), line(l) {}
    PURL::Url url;
    uint line;
  };
  QMap<uint, Data> _map;
...
public:
  ...
  virtual ~Process();
  virtual bool check() const { return Main::toolGroup().check(_data.device); }
  virtual bool start();
...
protected:
  ...
  virtual QString libraryPrefix() const { return QString::null; }
...
public:
  ...
  void processDone();
  void processFailed();
  void log(Log::Type type, const QString &message, const PURL::Url &url = PURL::Url(), uint line = 0);
  void log(Log::DebugLevel debug, const QString &message, const PURL::Url &url = PURL::Url(), uint line = 0);
