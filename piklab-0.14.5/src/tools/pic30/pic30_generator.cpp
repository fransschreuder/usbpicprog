...
SourceLine::List PIC30::SourceGenerator::configLines(PURL::ToolType type, const Device::Memory &memory, bool &ok) const
{
  ...
  for (uint i=0; i<data.nbWords(Pic::MemoryConfig); i++) {
    ...
    QString code;
    if ( type==PURL::Assembler ) code += "config __" + cword.name + ", ";
    else code += "_" + cword.name + "(";
    code += cnames.join(" & ");
    if ( type==PURL::Compiler ) code += ");";
...
{
  SourceLine::List lines;
  if ( type==PURL::Assembler ) lines.appendIndentedCode(".include \"p" + data.name().lower() + ".inc\"");
