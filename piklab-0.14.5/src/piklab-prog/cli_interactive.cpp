...
const CLI::PropertyData CLI::PROPERTY_DATA[] = {
  ...
  { "target-self-powered", "target-self-powered", 0, I18N_NOOP("Indicates that device is self-powered."), 0, 0 },
...
CLI::ExitCode CLI::Interactive::processLine(const QString &s)
{
  ...
  if ( words[0]=="get" ) {
    ...
    if ( !s.isEmpty() ) log(Log::Data, words[1] + ": " + s);
    return OK;
  }
  ...
  if ( words[0]=="break" ) {
    if ( words.count()==3 ) {
      if ( words[1]=="e" ) {
        ...
        if ( Breakpoint::list().contains(data) ) return okExit(i18n("Breakpoint already set at %1").arg(toHexLabel(address, nbChars(Number::Hex, address))));
        Breakpoint::list().append(data);
        Breakpoint::list().setAddress(data, address);
        Breakpoint::list().setState(data, Breakpoint::Active);
        return okExit(i18n("Breakpoint set at %1").arg(toHexLabel(address, nbChars(Number::Hex, address))));
      }
    ...
    if ( words.count()==1 ) {
      ...
      log(Log::Data, i18n("Breakpoints:"));
      uint nbc = 0;
      if (_device) nbc = _device->nbCharsAddress();
      else for (uint i=0; i<nb; i++) {
        uint address = Breakpoint::list().address(Breakpoint::list().data(i));
        nbc = QMAX(nbc, nbChars(Number::Hex, address));
      }
      for (uint i=0; i<nb; i++) {
        uint address = Breakpoint::list().address(Breakpoint::list().data(i));
        log(Log::Data, QString("  #%1: %2").arg(i).arg(toHexLabel(address, nbc)));
      }
      return OK;
    }
  ...
  if ( words[0]=="clear" ) {
    ...
    return okExit(i18n("Breakpoint at %1 removed").arg(toHexLabel(address, nbChars(Number::Hex, address))));
  }
  if ( words[0]=="raw-com" ) {
...
void CLI::Interactive::lineRead()
{
  ...
  _input = QString::null;
  if ( processLine(s)==EXITING ) return;
  QTimer::singleShot(0, this, SLOT(displayPrompt()));
}
...
CLI::ExitCode CLI::Interactive::registerList()
{
  ...
  log(Log::Data, i18n("Special Function Registers:"));
  QValueVector<Pic::RegisterNameData> list = Pic::sfrList(data);
  for (uint i=0; i<uint(list.count()); i++) log(Log::Data, QString("  %1: %2").arg(toHexLabel(list[i].data().address(), rdata.nbCharsAddress())).arg(list[i].label()));
  log(Log::Data, i18n("General Purpose Registers:"));
...
CLI::ExitCode CLI::Interactive::variableList()
{
  ...
  if ( list.count()==0 ) log(Log::Data, i18n("No variable"));
  for (uint i=0; i<uint(list.count()); i++) log(Log::Data, QString("  %1: %2").arg(toHexLabel(list[i].data().address(), rdata.nbCharsAddress())).arg(list[i].label()));
  return OK;
}

uint CLI::Interactive::findRegisterAddress(const QString &name)
{
  const Pic::Data &data = static_cast<const Pic::Data &>(*_device);
  const Coff::Object *coff = Debugger::manager->coff();
  const Pic::RegistersData &rdata = data.registersData();
  bool ok;
  ulong address = fromAnyLabel(name, &ok);
  if (ok) {
    if ( address>rdata.addressFromIndex(rdata.nbRegisters()-1) ) return Device::INVALID;
    return address;
  }
  ...
  if ( coff==0 ) return Device::INVALID;
...
CLI::ExitCode CLI::Interactive::executeRegister(const QString &name, const QString &value)
{
  ...
  if ( !ok ) return errorExit(i18n("Number format not recognized"), ARG_ERROR);
  if ( v>maxValue(Number::Hex, nbChars) ) return errorExit(i18n("The given value is too large (max: %1)").arg(toHexLabel(maxValue(Number::Hex, nbChars), nbChars)), ARG_ERROR);
  ...
  if ( name.lower()=="w" || name.lower()=="wreg" )
    ...
    uint address = findRegisterAddress(name);
    if ( address==Device::INVALID ) return errorExit(i18n("Unknown register or variable name"), ARG_ERROR);
...
CLI::ExitCode CLI::Interactive::executeRawCommands(const QString &filename)
{
  ...
  for (;;) {
    ...
    if (write) {
      ...
      if ( rs!=s ) log(Log::Warning, i18n("Read string is different than expected: %1 (%2)").arg(rs).arg(s));
    }
  }
