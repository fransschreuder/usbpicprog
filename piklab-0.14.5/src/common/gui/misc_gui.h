...
{
extern void information(const QString &text, Log::ShowMode show);
...
public:
  ...
  void appendItem(const QString &label, uint id) { appendItem(label, QPixmap(), id); }
  void appendItem(const QString &label, const QString &icon, uint id);
  void appendItem(const QString &label, const QPixmap &icon, uint id);
...
protected:
  ...
  virtual uint blankValue() const = 0;
};
...
public:
  ...
  void setValue(uint word) { _word = word; set(); }
...
protected:
  ...
  virtual uint mask() const { return maxValue(Number::Hex, _nbChars); }
  virtual uint normalizeWord(uint value) const { return value & mask(); }
  virtual uint word() const { return _word; }
  virtual void setWord(uint value) { _word = value; }
  virtual uint blankValue() const { return 0; }
};
