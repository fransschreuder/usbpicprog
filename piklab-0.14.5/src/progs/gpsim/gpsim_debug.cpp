...
bool GPSim::Debugger::setBreakpoints(const QValueList<uint> &list)
{
  for (uint i=0; i<_nbBreakpoints; i++)
    if ( !hardware()->execute("clear " + QString::number(i), true) ) return false;
  for (uint i=0; i<uint(list.count()); i++)
    if ( !hardware()->execute("break e 0x" + toHex(list[i], nbChars(list[i])), true) ) return false;
...
bool GPSim::Debugger::readWreg(uint &value)
{
...
bool GPSim::Debugger::getRegister(const QString &name, uint &value)
{
  QStringList lines;
  QRegExp r;
...
bool GPSim::Debugger::getRegister(uint address, uint &value)
{
...
bool GPSim::Debugger::readRegister(const Register::TypeData &data, uint &value)
{
...
bool GPSim::Debugger::setRegister(const QString &name, uint value)
{
  if ( hardware()->version()<VersionData(0, 22, 0) ) {
...
bool GPSim::Debugger::setRegister(uint address, uint value)
{
  const Pic::RegistersData &rdata = device()->registersData();
...
bool GPSim::Debugger::writeRegister(const Register::TypeData &data, uint value)
{
  if ( data.type()==Register::Special ) {
...
bool GPSim::Debugger::writeWreg(uint value)
{
...
void GPSim::Group::initSupported()
{
  ...
  for (uint i=0; i<uint(devices.count()); i++) {
    ...
    data.data = Device::lister().data(s);
    if (data.data) {
      if ( static_cast<const Pic::Data *>(data.data)->architecture()==Pic::Arch_18F && oldGpsim ) continue;
      addDevice(data);
    }
  }
}

Programmer::Hardware *GPSim::Group::createHardware(::Programmer::Base &base, const Port::Description &) const
{
