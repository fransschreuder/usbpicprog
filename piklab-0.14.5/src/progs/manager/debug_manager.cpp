...
bool Debugger::Manager::internalInit()
{
  delete _coff;
  _coff = 0;
  if ( !coffUrl().probablyExists() ) return false;
  Log::Base log;
...
uint Debugger::Manager::pc() const
{
  return (debugger() ? debugger()->pc() : Device::INVALID);
}
...
Breakpoint::MarkType Debugger::Manager::breakpointType(const Breakpoint::Data &data) const
{
  ...
  if ( address==Device::INVALID ) return Breakpoint::BreakpointInvalid;
...
bool Debugger::Manager::checkBreakpoint(const Breakpoint::Data &bdata, bool onlyWarn, uint &address)
{
  address = Device::INVALID;
  if ( _coff==0 ) return true;
  QValueVector<uint> addresses = _coff->addresses(bdata.url, bdata.line);
...
void Debugger::Manager::computeBreakpointAddresses()
{
  ...
  for (int i=Breakpoint::list().count()-1; i>=0; i--) {
    ...
    else if ( Breakpoint::list().address(data)!=Device::INVALID && nb>0 ) {
...
QValueList<uint> Debugger::Manager::activeBreakpointAddresses() const
{
  QValueList<uint> addresses;
  for (uint i=0; i<Breakpoint::list().count(); i++) {
    const Breakpoint::Data &data = Breakpoint::list().data(i);
...
bool Debugger::Manager::writeRegister(const Register::TypeData &data, uint value)
{
  Q_ASSERT( data.type()==Register::Regular || data.type()==Register::Special );
...
bool Debugger::Manager::prepareDebugging()
{
  if ( programmerGroup()->isSoftware() && programmerGroup()->isDebugger() ) {
    ...
    for (; i<Programmer::Nb_InputFileTypes; i++) {
      ...
      debugger()->setupInput(type, url.directory().path(false), url.filename());
      break;
    }
