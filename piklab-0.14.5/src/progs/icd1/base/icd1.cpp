...
bool Icd1::Hardware::sendCommand(uint cmd, uint *res, uint timeout)
{
...
bool Icd1::Hardware::getFirmwareVersion(VersionData &version)
{
  ...
  version = VersionData(v1 / 256, v1 & 0xFF, v2);
  return true;
}
...
bool Icd1::Hardware::readVoltages(VoltagesData &voltages)
{
  ...
  voltages[Pic::TargetVdd].value = (2.050 / 256) * res;  // voltage at AN0 pin
  ...
  voltages[Pic::TargetVpp].value = (2.050 / 256) * (res & 0xFF);  // voltage at AN1 pin
...
bool Icd1::Hardware::selfTest()
{
  ...
  if ( res!=0 ) {
    log(Log::Warning, i18n("Self-test failed (returned value is %1)").arg(res));
    return false;
  }
  return true;
}
...
bool Icd1::Hardware::gotoMemory(Pic::MemoryRangeType type, uint offset)
{
  ...
  uint cmd = (type==Pic::MemoryEeprom ? 0x7006 : 0x7005);
  if ( !sendCommand(cmd) )  return false;
  if ( type==Pic::MemoryConfig || type==Pic::MemoryUserId || type==Pic::MemoryDeviceId ) {
    Q_ASSERT( offset==0 );
    offset = device().range(type).start - 0x2000;
    if ( !sendCommand(0xC000 + offset) )  return false;
  } else if ( !sendCommand(0x2000 + offset) )  return false;
  return true;
}

bool Icd1::Hardware::readMemory(Pic::MemoryRangeType type, uint wordOffset, uint nbWords, Device::Array &data, const ::Programmer::VerifyData *vdata)
{
  if ( !gotoMemory(type, wordOffset) ) return false;
  data.resize(nbWords);
  for (uint i=0; i<nbWords; i++) if ( !sendCommand(0x7002, &data[i]) ) return false;
...
bool Icd1::Hardware::writeMemory(Pic::MemoryRangeType type, const Device::Array &data,
                                 uint wordOffset, uint nbWords)
{
  if ( !gotoMemory(type, wordOffset) ) return false;
  for (uint i=0; i<nbWords; i++) if ( !sendCommand(0x8000 | data[wordOffset+i]) ) return false;
  if ( !sendCommand(0x7001) ) return false; // end sequence
  return true;
}
...
bool Icd1::Hardware::writeRegister(uint address, uint value, uint nbBytes)
{
...
bool Icd1::Hardware::readRegister(uint address, uint &value, uint nbBytes)
{
  Q_ASSERT( nbBytes==1 || nbBytes==2 );
  // #### TODO
  return false;
...
uint Icd1::Hardware::getProgramCounter()
{
  // #### TODO
  return 0;
}
