...
//----------------------------------------------------------------------------
void Direct::pic16::send_bits(uint d, uint nbb)
{
    hardware().setWrite();
    for (uint x = nbb; x; --x) {
...
void Direct::pic16::send_word(uint d)
{
    hardware().setWrite();
...
uint Direct::pic16::readWord(Pic::MemoryRangeType type)
{
