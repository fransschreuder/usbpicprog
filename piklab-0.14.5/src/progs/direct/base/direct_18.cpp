...
bool Direct::P18::pulse(const char *&cmd, uint value, uint &res, bool &ok)
{
  switch (*cmd) {
    case 'x': {
      uint n;
...
void Direct::P18::send_word(uint d)
{
    hardware().setWrite();
    for (uint x = 0; x<16; x++) {
...
void Direct::P18::send_bits(uint d, uint nbb)
{
    hardware().setWrite();
    for (int x = nbb; x; --x) {
        hardware().setPin(Clock, High);
