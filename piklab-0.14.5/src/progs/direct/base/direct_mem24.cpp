...
uint Direct::Mem24DeviceSpecific::readByte(State ack)
{
  hardware().setRead();
  set(Low, High);
  uint b = 0;
...
bool Direct::Mem24DeviceSpecific::writeByteAck(uint value)
{
  bool acked;
...
bool Direct::Mem24DeviceSpecific::writeByte(uint value, bool &acked)
{
  Q_ASSERT( value<=0xFF );
  hardware().setWrite();
  set(Low, Low);
