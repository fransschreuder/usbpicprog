...
Direct::HardwareConfigWidget::HardwareConfigWidget(::Programmer::Base &base, QWidget *parent, bool edit)
...
void Direct::HardwareConfigWidget::sendBits()
{
...
uint Direct::HardwareConfigWidget::pin(PinType ptype) const
{
  return static_cast<const Hardware *>(_hardware)->pinForIndex(PIN_DATA[ptype].dir, _combos[ptype]->currentItem());
}

void Direct::HardwareConfigWidget::slotPinChanged()
{
  for (uint i = 0; i<Nb_PinTypes; i++) {
...
void Direct::HardwareConfigWidget::updatePin(PinType ptype)
{
...
void Direct::HardwareConfigWidget::slotTestPin()
{
  for (uint i = 0; i<Nb_PinTypes; i++) {
...
void Direct::HardwareConfigWidget::updateTestPin(PinType ptype)
{
...
void Direct::HardwareConfigWidget::sendBits(uint d, int nbb)
{
  Q_ASSERT(_connected);
  hardware()->setWrite();
  for (; nbb; --nbb) {
...
void Direct::HardwareConfigWidget::slotSendBits()
{
    sendBits(0xA55A, 16);
}

bool Direct::HardwareConfigWidget::set(const Port::Description &pd, const ::HProgrammer::HardwareData &data)
{
  // connect port
...
HProgrammer::HardwareData *Direct::HardwareConfigWidget::data() const
{
...
Direct::ConfigWidget::ConfigWidget(const ::Programmer::Group &group, QWidget *parent)
  : ::HProgrammer::ConfigWidget(new ::Direct::PicBase(group, 0), new Config, parent, "direct_config_widget")
{}

HProgrammer::HardwareConfigWidget *Direct::ConfigWidget::createHardwareConfigWidget(QWidget *parent, bool edit) const
{
