...
bool Pickit2::USBPort::readFirmwareCodeMemory(Device::Array &data, const Device::Array *vdata)
{
  const Pic::Data *device = static_cast<const Pic::Data *>(Device::lister().data("18F2455"));
  log(Log::NormalDebug, "  Read pickit2 firmware");
  for (uint i=0; i<data.count(); i+=16) { // 2 bytes/word and 32 bytes/command max
    uint start = 0;
    for (; start<16; start++) if ( data[i + start]!=Device::INVALID ) break;
    if ( start==16 ) continue;
    uint end = 15;
    for (; end>start; end--) if ( data[i + end]!=Device::INVALID ) break;
    uint nb = end - start + 1;
    ...
    for (uint k=0; k<nb; k++) {
      ...
      if ( vdata && index>=0x1000 && index<0x3FF0 && data[index]!=(*vdata)[index] ) {
        log(Log::Error, i18n("Firmware memory does not match hex file (at address 0x%2: reading 0x%3 and expecting 0x%4).")
                    .arg(QString(toHex(index/2, device->nbCharsAddress())))
                    .arg(QString(toHex(data[index], device->nbCharsWord(Pic::MemoryCode))))
                    .arg(QString(toHex((*vdata)[index], device->nbCharsWord(Pic::MemoryCode)))));
         return false;
      }
    }
  }
...
{
  log(Log::NormalDebug, "  Erase pickit2 firmware");
...
bool Pickit2::USBPort::writeFirmwareCodeMemory(const Device::Array &data)
{
  log(Log::NormalDebug, "  Write pickit2 firmware");
  for (uint i=0x1000; i<data.count(); i+=16) { // 2 bytes/word and 32 bytes/command max
    ...
    for (uint k=0; k<nb; k++) {
      cmd[5 + 2*k] = data[i + start + k] & 0xFF;
      cmd[6 + 2*k] = (data[i + start + k] >> 8) & 0xFF;
    }
    if ( !command(cmd) ) return false;
    Port::usleep(100000);
  }
  Device::Array read;
  if ( !readFirmwareCodeMemory(read, &data) ) return false;
  log(Log::NormalDebug, "  Write pickit2 firmware loaded key");
...
bool Pickit2::USBPort::uploadFirmware(const Pic::Memory &memory)
{
  if ( !eraseFirmwareCodeMemory() ) return false;
  Device::Array data = memory.arrayForWriting(Pic::MemoryCode);
  return writeFirmwareCodeMemory(data);
}

//-----------------------------------------------------------------------------
bool Pickit2::Hardware::readVoltages(VoltagesData &voltages)
{
  log(Log::ExtraDebug, QString("readVoltages: Firmware is %1").arg(_base.firmwareVersion().pretty()));
  if ( _base.firmwareVersion()<VersionData(1, 20, 0) ) {
    log(Log::Warning, i18n("Cannot read voltages with this firmware version."));
    return true;
  }
...
{
  log(Log::ExtraDebug, QString("setVddVpp: Firmware is %1").arg(_base.firmwareVersion().pretty()));
  if ( _base.firmwareVersion()<VersionData(1, 20, 0) ) return true;
  log(Log::NormalDebug, QString("  set Vdd = %1 V and Vpp = %2 V").arg(vdd).arg(vpp));
  Array cmd;
  cmd[0] = 's';
...
{
  if ( type==Pic::MemoryEeprom ) return doEraseCommand(0x84, 0x00);
...
bool Pickit2::P18F::doRead(Pic::MemoryRangeType type, Device::Array &data, const ::Programmer::VerifyData *vdata)
{
  ...
  switch (type) {
    ...
    case Pic::MemoryConfig: {
      Pickit::Array cmd = port.createCommand(0x80, data.count(), device().range(type).start);
      if ( !port.command(cmd) ) return false;
...
bool Pickit2::P18F::doWrite(Pic::MemoryRangeType type, const Device::Array &data, bool force)
{
  ...
  switch (type) {
    case Pic::MemoryUserId: {
      Pickit::Array cmd = port.createCommand(0x82, data.count() / 2, device().range(type).start);
      for (uint i=0; i<data.count(); i++) cmd[i + 5] = data[i] & 0xFF;
    ...
    case Pic::MemoryConfig: {
      ...
      for (uint i=0; i<data.count()/2; i++) {
        cmd[4*i]     = 0x84;
        cmd[1 + 4*i] = 2*i;
        cmd[2 + 4*i] = data[2*i] & 0xFF;
        cmd[3 + 4*i] = data[2*i+1] & 0xFF;
      }
      return port.command(cmd);
    }
    case Pic::MemoryCode: {
      ...
      for (uint i=0; i<data.count();) {
        ...
        for (uint k=0; k<nb; k++) {
          if ( (data[i] & 0xFFFF)!=0xFFFF ) allBlank = false;
          cmd[5 + 2*k] = data[i] & 0xFF;
          cmd[6 + 2*k] = (data[i] >> 8) & 0xFF;
          i++;
        }
    ...
    case Pic::MemoryEeprom:
      for (uint i=0; i<data.count();) {
        ...
        for (uint k=0; k<32; k++) {
          if ( (data[i] & 0xFF)!=0xFF ) allBlank = false;
          cmd[4+k] = data[i];
          i++;
        }
        if ( !allBlank && !port.command(cmd) ) return false;
...
{
  if ( type==Pic::MemoryEeprom ) return doEraseCommand(0x81);
  Q_ASSERT( type==Pic::MemoryCode );
