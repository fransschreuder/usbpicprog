...
Icd2::USBPort::USBPort(uint deviceId, Log::Base &log)
  : Port::USB(log), _dataSend(false)
{
  init(Microchip::VENDOR_ID, deviceId);
}
...
bool Icd2::USBPort::connectDevice(const QString &mode)
{
  ...
  for (uint i=0; true; i++) {
    if ( i==10 ) {
      log(Log::Error, i18n("Problem connecting ICD2: please try again after unplug-replug."));
      return false;
    }
    _ctype = 0x02;
...
bool Icd2::USBPort::poll(uint size, QMemArray<uchar> &a)
{
  ...
  //log(Log::MaxDebug, QString("Receiced: \"%1\"").arg(toPrintable((const char *)a.data(), size)));
  return true;
}
...
bool Icd2::USBHardware::internalConnect(const QString &mode)
{
  ...
  log(Log::ExtraDebug, QString("need firmware ? %1").arg(USBPort::findDevice(Microchip::VENDOR_ID, ID_FIRMWARE)!=0));
  if ( Port::USB::findDevice(Microchip::VENDOR_ID, ID_FIRMWARE) ) {
    ...
    port.close();
    for (uint i=0; i<10; i++) {
      log(Log::ExtraDebug, QString("client here ? %1").arg(USBPort::findDevice(Microchip::VENDOR_ID, ID_CLIENT)!=0));
