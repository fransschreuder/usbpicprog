...
{
  switch (type) {
    case Pic::MemoryCode:             return "47";
    case Pic::MemoryEeprom:           return "48";
    case Pic::MemoryConfig:           return "49";
    case Pic::MemoryUserId:           return "4A";
    case Pic::MemoryDeviceId:
      if ( device().architecture()==Pic::Arch_30X ) return "49";
      return "4A";
    case Pic::MemoryCal:
    case Pic::MemoryCalBackup:
      if ( device().architecture()==Pic::Arch_16X ) return "49"; // ?
      return "47"; // for baseline only ?
    case Pic::MemoryDebugVector:      return "40";
...
{
  switch (type) {
    case Pic::MemoryCode:             return "43";
    case Pic::MemoryEeprom:           return "44";
    case Pic::MemoryConfig:           return "45";
    case Pic::MemoryUserId:           return "46";
    case Pic::MemoryDeviceId:         break;
    case Pic::MemoryCal:
    case Pic::MemoryCalBackup:
      if ( device().architecture()==Pic::Arch_16X ) return "45"; // ?
      return "43"; // for baseline only ?
    case Pic::MemoryDebugVector:      return "41";
    case Pic::MemoryProgramExecutive:
    case Pic::MemoryHardwareStack:
    case Pic::Nb_MemoryRangeTypes:    break;
  }
...
bool Icd2::Hardware::uploadFirmware(const Pic::Memory &memory)
{
  ...
  uint nbBytesWord = memory.device().nbBytesWord(Pic::MemoryCode); // should be 2 for 16F876
...
bool Icd2::Hardware::setTarget()
{
  ...
  // set code range end
  uint end = device().range(Pic::MemoryCode).end;
  if ( device().range(Pic::MemoryCal).start==end ) end++;
...
{
  // ??
  if ( device().architecture()==Pic::Arch_30X )
...
bool Icd2::Hardware::readMemory(Pic::MemoryRangeType type, uint wordOffset, uint nbWords,
                                Device::Array &data, const ::Programmer::VerifyData *vdata)
{
  ...
  uint div = 2;
  if ( type==Pic::MemoryEeprom || nbBytesWord>=2 ) div = 1;
  uint inc = device().addressIncrement(type);
  uint start = device().range(type).start; // address
  uint todo = inc * nbWords; // address
  ...
  do {
    ...
    offset += size;
    todo -= size;
    if ( type==Pic::MemoryCode || type==Pic::MemoryEeprom ) addProgress(nb);
  } while ( todo!=0 );
  return true;
}

bool Icd2::Hardware::writeBlock(uint nbBytesWord, const Device::Array &data,
                                uint wordOffset, uint nbWords)
{
  //qDebug("writeBlock offset:%s nbWords:%s (size: %s)", toHex(wordOffset, 4).data(), toHex(nbWords, 4).data(), toHex(data.size(), 4).data());
  Q_ASSERT( wordOffset+nbWords<=data.size() );
  // prepare data
  QString s = "{";
  uchar chk = 0;
  for (uint i=0; i<nbWords; i++) {
    QString ts = toHex(data[wordOffset+i], 2*nbBytesWord);
    for (int k=nbBytesWord-1; k>=0; k--) {
...
bool Icd2::Hardware::writeMemory(Pic::MemoryRangeType type, const Device::Array &data,
                       uint wordOffset, uint nbWords)
{
  ...
  do {
    ...
    QString cmd = w + toHex(start+offset, 8) + toHex(nb/div, 8);
    if ( !command(cmd, 0) ) return false;
    if ( !writeBlock(nbBytesWord, data, offset/inc, nb) ) return false;
    if ( !receiveResponse(cmd, 0, false) ) return false;
    offset += size;
    todo -= size;
    if ( type==Pic::MemoryCode || type==Pic::MemoryEeprom ) addProgress(nb);
...
bool Icd2::Hardware::writeRegister(uint address, uint value, uint nbBytes)
{
  QString cmd = "1B" + toHex(address, 8) + toHex(nbBytes, 8);
  if ( !command(cmd, 0) ) return false;
  Device::Array data(nbBytes);
  for (uint i=0; i<nbBytes; i++) {
    data[nbBytes-i-1] = value & 0xFF;
    value >>= 8;
  }
  Q_ASSERT( value==0 );
...
bool Icd2::Hardware::readRegister(uint address, uint &value, uint nbBytes)
{
...
uint Icd2::Hardware::getProgramCounter()
{
