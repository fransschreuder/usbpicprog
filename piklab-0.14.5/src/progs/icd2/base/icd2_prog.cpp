...
bool Icd2::ProgrammerBase::internalSetupHardware()
{
  ...
  if ( !_targetSelfPowered && device()->architecture()==Pic::Arch_30X
...
bool Icd2::ProgrammerBase::setupFirmware()
{
  ...
  if ( files.isEmpty() ) {
    log(Log::Error, i18n("Could not find firmware file \"%1\" in directory \"%2\".").arg(nameFilter).arg(dir.path(false)));
    return false;
  }
...
Programmer::Hardware *Icd2::Group::createHardware(::Programmer::Base &base, const Port::Description &pd) const
{
  if ( pd.type==Port::SerialType ) return new SerialHardware(base, pd.device);
