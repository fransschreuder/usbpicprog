...
bool Debugger::Base::halt()
{
  ...
  uint pc = Register::list().value(pcTypeData());
  log(Log::Information, QString("Halted at %1").arg(toLabel(Number::Hex, pc, _programmer.device()->nbCharsAddress())));
  _programmer.setState(::Programmer::Halted);
  return true;
}
