 ...
 ***************************************************************************/
#include "usbpicprog_pic.h"

#include "common/common/misc.h"

//-----------------------------------------------------------------------------
Usbpicprog::PulseEngine::PulseEngine(::Programmer::Base &base)
  : _base(base)
{
  _clockDelay = 0;
}

uint Usbpicprog::PulseEngine::pulseEngine(const QString &cmd, uint value, bool *ok)
{
...
bool Usbpicprog::PulseEngine::pulse(const char * &cmd, uint, uint &, bool &ok)
{
  ok = true;
  switch (*cmd) {
    case ';':
...
//-----------------------------------------------------------------------------
Usbpicprog::PicDeviceSpecific::PicDeviceSpecific(::Programmer::Base &base)
...
// UPPER case means signal High - lower case means Low
// b/B = burn (vpp)
bool Usbpicprog::PicDeviceSpecific::pulse(const char *&cmd, uint value, uint &res, bool &ok)
{
  switch (*cmd) {
    case 'b':
...
bool Usbpicprog::PicDeviceSpecific::setPowerOnVddFirst()
{
  setPowerOff();
...
bool Usbpicprog::PicDeviceSpecific::setPowerOnVppFirst()
{
  setPowerOff();
...
bool Usbpicprog::PicDeviceSpecific::setPowerOff()
{
...
bool Usbpicprog::PicDeviceSpecific::setTargetPowerOn(bool on)
{
  pulseEngine(on ? "P" : "p");
  return true;
}
...
bool Usbpicprog::Pic8DeviceSpecific::pulse(const char *&cmd, uint value, uint &res, bool &ok)
{
  switch (*cmd) {
    case 'S':
