 ...
 ***************************************************************************/
#include "usbpicprog_18F.h"

#include "usbpicprog_data.h"

//-----------------------------------------------------------------------------
bool Usbpicprog::P18F::skipMaskWords(Pic::MemoryRangeType type, const Device::Array &data,
...
void Usbpicprog::P18F::program(Type type)
{
  QString cmd;
  switch (type) {
    case Code:
    ...
    case Eeprom:
      for (;;) {
        ...
        pulseEngine("k0,X0000,"); // NOP
        uint b = get_byte();
        if ( (b & 0x02)==0 ) break; // WR bit clear
      }
...
void Usbpicprog::P18F::setCodePointer(uint address)
{
...
void Usbpicprog::P18F::setPointer(Pic::MemoryRangeType type, uint offset)
{
  if ( type==Pic::MemoryEeprom ) {
    pulseEngine("k0,S,k0,X6EA9,", 0x0E00 | (offset & 0x00FF));
    pulseEngine("k0,S,k0,X6EAA,", 0x0E00 | ((offset & 0xFF00) >> 8));
  } else setCodePointer(device().range(type).start + offset);
}

void Usbpicprog::P18F::directAccess(Pic::MemoryRangeType type)
{
  if ( type==Pic::MemoryCode || type==Pic::MemoryUserId || type==Pic::MemoryEeprom ) pulseEngine("k0,X9CA6"); // unset EECON1:CFGS
  else pulseEngine("k0,X8CA6"); // set EECON1:CFGS
  if ( type==Pic::MemoryEeprom ) pulseEngine("k0,X9EA6"); // unset EECON1::EEPGD
  else pulseEngine("k0,X8EA6"); // set EECON1::EEPGD
}

bool Usbpicprog::P18F::doRead(Pic::MemoryRangeType type, Device::Array &data, const ::Programmer::VerifyData *vdata)
{
...
bool Usbpicprog::P18F::doWrite(Pic::MemoryRangeType type, const Device::Array &data, bool force)
{
  ...
  switch (type) {
    case Pic::MemoryUserId: {
      setPointer(type, 0);
      Q_ASSERT( device().nbWords(Pic::MemoryUserId)==8 );
      uint i = 0;
      for (uint k=0; k<4; k++) {
        uint word = data[i] | data[i+1] << 8;
    ...
    case Pic::MemoryCode: {
      uint progWidth = device().nbWordsCodeWrite();
    ...
    case Pic::MemoryEeprom:
      for(uint i = 0; i<data.count(); ) {
        ...
        setPointer(type, inc * i);
        pulseEngine("k0,S,k0,X6EA8,", 0x0E00 | data[i]); // load data
...
void Usbpicprog::P18F::unlockEeprom()
{
  pulseEngine("k0,X0E55,k0,X6EA7,k0,X0EAA,k0,X6EA7,");
}

bool Usbpicprog::P18F::doEraseCommand(uint cmd1, uint cmd2)
{
  if ( cmd1!=0 ) {
...
bool Usbpicprog::P18F::doEraseRange(Pic::MemoryRangeType type)
{
  ...
  if ( type==Pic::MemoryEeprom ) return doEraseCommand(0, 0x0081);
  return false;
}

bool Usbpicprog::P18F::doErase(bool)
{
  return doEraseCommand(0, 0x0080);
}

//-----------------------------------------------------------------------------
void Usbpicprog::P18F1220::program(Type type)
{
  if ( type==Eeprom ) {
    pulseEngine("k0,X0000,"); // NOP
...
//-----------------------------------------------------------------------------
void Usbpicprog::P18F242::configureSinglePanel()
{
...
//-----------------------------------------------------------------------------
bool Usbpicprog::P18F2539::doErase(bool)
{
  // apparently there is no chip erase...
  return ( doEraseRange(Pic::MemoryCode) && doEraseRange(Pic::MemoryEeprom) );
}

bool Usbpicprog::P18F2539::doEraseCommand(uint cmd1, uint cmd2)
{
  Q_ASSERT( cmd1==0 );
  setCodePointer(0x3C0004);
...
//-----------------------------------------------------------------------------
bool Usbpicprog::P18F2439::doEraseRange(Pic::MemoryRangeType type)
{
  if ( type==Pic::MemoryCode ) {
    configureSinglePanel();
    directAccess(Pic::MemoryCode);
    for (uint i=0; i<40; i++) {
...
//-----------------------------------------------------------------------------
bool Usbpicprog::P18F2221::doEraseRange(Pic::MemoryRangeType type)
{
  if ( type==Pic::MemoryCode ) {
  ...
  if ( type==Pic::MemoryEeprom ) return doEraseCommand(0, 0x8484);
  return false;
}

bool Usbpicprog::P18F2221::doErase(bool)
{
  return doEraseCommand(0x0F0F, 0x8787); // chip erase
}

//-----------------------------------------------------------------------------
bool Usbpicprog::P18F6527::doErase(bool)
{
  return doEraseCommand(0xFFFF, 0x8787); // chip erase
}

//-----------------------------------------------------------------------------
bool Usbpicprog::P18F6310::doErase(bool)
{
  setCodePointer(0x3C0005);
