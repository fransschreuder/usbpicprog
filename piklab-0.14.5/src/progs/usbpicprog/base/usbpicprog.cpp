...
//-----------------------------------------------------------------------------
const Usbpicprog::PinData Usbpicprog::PIN_DATA[Nb_PinTypes] = {
...
//-----------------------------------------------------------------------------
Usbpicprog::Hardware::Hardware(::Programmer::Base &base, Port::Base *port, const HData &data)
  : ::Programmer::PicHardware(base, port), _data(data)
{}

bool Usbpicprog::Hardware::internalConnectHardware()
{
...
void Usbpicprog::Hardware::setPin(PinType type, bool on)
{
...
bool Usbpicprog::Hardware::readBit()
{
...
uint Usbpicprog::Hardware::nbPins(Port::IODir dir) const
{
  return _port->pinData(dir).count();
}

QString Usbpicprog::Hardware::pinLabelForIndex(Port::IODir dir, uint i) const
{
  Port::PinData pd = _port->pinData(dir)[i];
  return QString("%1 (%2)").arg(pd.pin+1).arg(pd.label);
}

Port::IODir Usbpicprog::Hardware::ioTypeForPin(int pin) const
{
...
uint Usbpicprog::Hardware::pinForIndex(Port::IODir dir, uint i) const
{
...
uint Usbpicprog::Hardware::indexForPin(Port::IODir dir, int pin) const
{
...
bool Usbpicprog::Hardware::isGroundPin(int pin) const
{
  Q_ASSERT( pin!=0 );
  uint p = (pin<0 ? -pin : pin)-1;
...
bool Usbpicprog::operator !=(const HData &d1, const HData &d2)
{
...
//-----------------------------------------------------------------------------
Usbpicprog::SerialHardware::SerialHardware(::Programmer::Base &base, const QString &portDevice, const HData &data)
  : Hardware(base, new SerialPort(portDevice, base), data)
{}

Usbpicprog::ParallelHardware::ParallelHardware(::Programmer::Base &base, const QString &portDevice, const HData &data)
  : Hardware(base, new Port::Parallel(portDevice, base), data)
{}

/*
//-----------------------------------------------------------------------------
int Usbpicprog::Hardware::hardware_test()
{
    char t[80] ;

    cout << endl << "Please execute this Usbpicprog::Hardware test without any PIC"
    ...
    cout << "Congratulations! - Usbpicprog::Hardware is OK." <<endl ;
    return 0 ;
}

// this is my specific speed test
int Usbpicprog::Hardware::timing_test()
{
    char t[80] ;
    int cmd ;
    long loop = 50000000 ;
