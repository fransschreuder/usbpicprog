...
Usbpicprog::Mem24DeviceSpecific::Mem24DeviceSpecific(::Programmer::Base &base)
  : ::Programmer::Mem24DeviceSpecific(base)
{}

bool Usbpicprog::Mem24DeviceSpecific::setPowerOn()
{
...
bool Usbpicprog::Mem24DeviceSpecific::setPowerOff()
{
  hardware().setPin(Clock, Low);
  hardware().setPin(DataOut, Low);
  hardware().setPin(Vpp, Off);
...
bool Usbpicprog::Mem24DeviceSpecific::verifyPresence()
{
...
uint Usbpicprog::Mem24DeviceSpecific::controlByte(uint address, Operation operation) const
{
...
bool Usbpicprog::Mem24DeviceSpecific::setAddress(uint address)
{
...
bool Usbpicprog::Mem24DeviceSpecific::doRead(Device::Array &data, const ::Programmer::VerifyData *vdata)
{
...
bool Usbpicprog::Mem24DeviceSpecific::doWrite(const Device::Array &data)
{
  QTime time;
...
void Usbpicprog::Mem24DeviceSpecific::set(State clock, State data)
{
...
void Usbpicprog::Mem24DeviceSpecific::setData(State data)
{
  set(Low, data);
  set(High, data);
  set(Low, data);
...
uint Usbpicprog::Mem24DeviceSpecific::readByte(State ack)
{
  hardware().setRead();
  set(Low, High);
  uint b = 0;
...
bool Usbpicprog::Mem24DeviceSpecific::writeByteAck(uint value)
{
  bool acked;
...
bool Usbpicprog::Mem24DeviceSpecific::writeByte(uint value, bool &acked)
{
  Q_ASSERT( value<=0xFF );
  hardware().setWrite();
  set(Low, Low);
  for (int i=7; i>=0; i--) setData(value & (1<<i) ? High : Low);
  hardware().setRead();
  set(Low, High);
  set(High, High);
...
bool Usbpicprog::Mem24DeviceSpecific::start()
{
  hardware().setWrite();
  set(Low, High);
  set(High, High);
...
bool Usbpicprog::Mem24DeviceSpecific::stop()
{
  hardware().setWrite();
  set(Low, Low);
  set(High, Low);
