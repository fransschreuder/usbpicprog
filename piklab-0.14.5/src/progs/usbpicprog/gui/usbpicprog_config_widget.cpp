 ...
 ***************************************************************************/
#include "usbpicprog_config_widget.h"

#include <qtooltip.h>
#include <qcheckbox.h>
#include <qtimer.h>

#include "progs/usbpicprog/base/usbpicprog_prog_config.h"
#include "progs/usbpicprog/base/usbpicprog_prog.h"

//-----------------------------------------------------------------------------
::Programmer::ConfigWidget *Usbpicprog::GroupUI::createConfigWidget(QWidget *parent) const
{
...
Usbpicprog::HardwareConfigWidget::HardwareConfigWidget(::Programmer::Base &base, QWidget *parent, bool edit)
  : ::HProgrammer::HardwareConfigWidget(base, parent, edit)
{
  // pins assignment
...
void Usbpicprog::HardwareConfigWidget::sendBits()
{
...
uint Usbpicprog::HardwareConfigWidget::pin(PinType ptype) const
{
  return static_cast<const Hardware *>(_hardware)->pinForIndex(PIN_DATA[ptype].dir, _combos[ptype]->currentItem());
}

void Usbpicprog::HardwareConfigWidget::slotPinChanged()
{
  for (uint i = 0; i<Nb_PinTypes; i++) {
...
void Usbpicprog::HardwareConfigWidget::updatePin(PinType ptype)
{
...
void Usbpicprog::HardwareConfigWidget::slotTestPin()
{
  for (uint i = 0; i<Nb_PinTypes; i++) {
...
void Usbpicprog::HardwareConfigWidget::updateTestPin(PinType ptype)
{
...
void Usbpicprog::HardwareConfigWidget::updateTestStatus(PinType ptype, bool on)
{
  if (on) _testLabels[ptype]->setText(i18n(PIN_DATA[ptype].onLabel));
  else _testLabels[ptype]->setText(i18n(PIN_DATA[ptype].offLabel));
}

void Usbpicprog::HardwareConfigWidget::updateDataIn()
{
...
void Usbpicprog::HardwareConfigWidget::sendBits(uint d, int nbb)
{
  Q_ASSERT(_connected);
  hardware()->setWrite();
  for (; nbb; --nbb) {
...
void Usbpicprog::HardwareConfigWidget::slotSendBits()
{
    sendBits(0xA55A, 16);
}

bool Usbpicprog::HardwareConfigWidget::set(const Port::Description &pd, const ::HProgrammer::HardwareData &data)
{
  // connect port
  ...
  if ( pd.type==Port::SerialType ) _hardware = new SerialHardware(_base, pd.device, hdata.data);
  else _hardware = new ParallelHardware(_base, pd.device, hdata.data);
...
HProgrammer::HardwareData *Usbpicprog::HardwareConfigWidget::data() const
{
...
//-----------------------------------------------------------------------------
Usbpicprog::ConfigWidget::ConfigWidget(const ::Programmer::Group &group, QWidget *parent)
  : ::HProgrammer::ConfigWidget(new ::Usbpicprog::PicBase(group, 0), new Config, parent, "usbpicprog_config_widget")
{}

HProgrammer::HardwareConfigWidget *Usbpicprog::ConfigWidget::createHardwareConfigWidget(QWidget *parent, bool edit) const
{
  return new HardwareConfigWidget(*_base, parent, edit);
}
